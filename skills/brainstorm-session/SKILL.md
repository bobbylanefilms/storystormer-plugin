...
8b. **Setup/payoff chains go to one of two places, never a ledger.** When a chain surfaces in conversation, apply primer §2's admission criteria: **load-bearing** (the payoff fails without the setup), **long-range** (more than about two chapters), **not self-evident** (wouldn't be caught by reading adjacent chapters). All three → it is a §2 Reveal Architecture entry; log it as a decision with its plant and payoff chapters so `treatment-update` registers it. Fewer than three → it lives in the treatment as ordinary narrative. A chain that has been planted *and* paid is struck from §2, not archived. When a later chapter needs a payoff and prose already exists, **search the harvest first**: glob `chapters/*/ch*-notes.md` and read the Harvest sections for a particular already on the page before inventing a new one. That is how dense setup/payoff webs actually get built.
