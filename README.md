...
- **The `blueprint` skill — pre-prose production briefs.** For any outlined chapter, Blueprint assembles a single self-contained document a prose agent can write from alone: every surfacing character tiered to the right resolution (POV → Major → Supporting → Minor → Referenced, with word ceilings and their personality frameworks carried), every worldbuilding element the chapter touches at prominence-tiered fidelity, scene-current state fused in, across nine sections (Scene Function, Story Context, Characters, Setting, Conflict, Symbolism, Continuity, Worldbuilding, Other Notes). Because the Blueprint replaces the raw Canon *and* the primer at prose time, it's a fidelity-preserving consolidation — inclusion is generous; what's tiered is resolution. It's the folder-native port of the web app's Blueprint refinement pass — where the app runs a bounded agentic read-tool loop against a database, the skill runs that loop natively over the story folder, batch mode dispatching one Blueprint subagent per chapter in parallel. Lands in the slot the v0.5.0 architecture reserved: `chapters/chapter-NN/ch<NN>-blueprint.md`, pipeline stage `outline → blueprint → prose → notes`.
